...
public class GamePanel extends JPanel implements ActionListener{
		...
		if(running) {
			...
			for(int i=0;i<bodyParts;i++) {
				if(i==0) {
					...
					/*
					this makes the snake body one solid color
	...
	public void gameOver(Graphics g) {
		...
		g.drawString("Game Over", 
				(SCREEN_WIDTH - metricsGameOver.stringWidth(
						"Game Over"))/2, SCREEN_HEIGHT/2);
		// Restart Text
		g.setFont(new Font("Ink Free", Font.BOLD, 50));
		FontMetrics metricsRestart = getFontMetrics(g.getFont());
		g.drawString("Press Enter to Restart", 
				(SCREEN_WIDTH - metricsRestart.stringWidth(
						"Press Enter to Restart"))/2, (SCREEN_HEIGHT/2) + 75);
